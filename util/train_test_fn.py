...
def train(epoch,model,loss_fn,optimizer,dataloader,pair_generation_tnf,gridGen,vis,use_cuda=True,log_interval=50):
...
def test(epoch,model,loss_fn,dataloader,pair_generation_tnf,gridGen,vis,use_cuda=True,log_interval=50):
    model.eval()
    test_loss =0
    for batch_idx, batch in enumerate(dataloader):
    ...
    print('Test set: Average loss: {:.4f}'.format(train_loss))
    vis.drawLoss(epoch,train_loss)
