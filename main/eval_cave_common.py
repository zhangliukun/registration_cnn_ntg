...
from main.test_mulit_images import createModel, compute_average_grid_loss, compute_correct_rate
...
def createDataloader(image_path,single_channel = False,batch_size = 16,use_cuda=True):
    ...
    pair_generator = HarvardDataPair(single_channel= single_channel ,use_cuda=use_cuda)

    return dataloader,pair_generator

def iterDataset(dataloader,pair_generator,ntg_model,vis,threshold=10,use_cuda=True,use_traditional = False,use_combine = False,save_mat = False,use_cnn = False):
    '''
    迭代数据集中的批次数据，进行处理
    :param dataloader:
    ...
    for batch_idx,batch in enumerate(dataloader):
        ...
        if use_traditional:
            with torch.no_grad():
                ...
                ntg_param_batch = estimate_aff_param_iterator(target_image_batch[:, 0, :, :].unsqueeze(1),
        ...
        vis.showImageBatch(source_image_batch,normailze=normailze_visual,win='source_image_batch',title='source_image_batch')
        vis.showImageBatch(target_image_batch,normailze=normailze_visual,win='target_image_batch',title='target_image_batch')
        vis.showImageBatch(ntg_wraped_image,normailze=normailze_visual,win='ntg_wraped_image',title='ntg_wraped_image')
...
if __name__ == '__main__':
    ...
    iterDataset(dataloader, pair_generator, ntg_model, vis,
