...
def compute_image_pyramid(image,f,nL,ration):
    ...
    for m in range(1,nL):
        ...
        im1 = np.array(Image.fromarray(tmp[:,:,0]).resize((int(tmp[:,:,0].shape[0]*ration),int(tmp[:,:,0].shape[1]*ration)),resample=BICUBIC))
        im2 = np.array(Image.fromarray(tmp[:,:,1]).resize((int(tmp[:,:,1].shape[0]*ration),int(tmp[:,:,1].shape[1]*ration)),resample=BICUBIC))
